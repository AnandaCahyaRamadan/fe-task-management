      ...
      <vue-good-table
        ...
        <template #table-row="{ column, row }">
          <span v-if="column.field === 'actions'">
            <button @click="editMember(row)" class="text-primary font-semibold py-1 px-1 rounded-xl">
              <i class="fas fa-edit"></i>
            </button>
            <button @click="deleteMember(row.id)" class="text-red-500 font-semibold py-1 px-1 rounded-xl">
...
export default {
  ...
  setup() {
    const members = ref([]);
    const form = ref({ name: "", email: "", password: "", password_confirmation: "", image: null });
    ...
    const membersWithIndex = computed(() =>
      members.value.map((m, i) => ({ ...m, index: i + 1 }))
    );
    ...
    const resetForm = () => {
      editId.value = null;
      form.value = { name: "", email: "", password: "", password_confirmation: "", image: null };
      errors.value = {};
    };
    ...
    const submitMember = async () => {
      ...
      try {
        if (editId.value) {
          ...
          await axios.post(`/member/${editId.value}`, data, { headers: { "Content-Type": "multipart/form-data" } });
          Swal.fire({ toast: true, position: "top-end", icon: "success", title: "Member berhasil diupdate", showConfirmButton: false, timer: 1000, timerProgressBar: true });
        } else {
          await axios.post("/member", data, { headers: { "Content-Type": "multipart/form-data" } });
          Swal.fire({ toast: true, position: "top-end", icon: "success", title: "Member berhasil ditambahkan", showConfirmButton: false, timer: 1000, timerProgressBar: true });
        }
        resetForm();
        closeModal();
        await fetchMembers();
        ...
        if (error.response?.status === 422) {
          ...
          console.error(error.response?.data);
          Swal.fire({ toast: true, position: "top-end", icon: "error", title: "Terjadi kesalahan", showConfirmButton: false, timer: 1000, timerProgressBar: true });
        }
      } finally {
        loading.value = false;
      }
    ...
    const deleteMember = async (id) => {
      ...
      try {
        ...
        await nextTick();
        Swal.fire({ toast: true, position: "top-end", icon: "success", title: "Member berhasil dihapus", showConfirmButton: false, timer: 1000, timerProgressBar: true });
      } catch (error) {
        console.error(error.response?.data);
        Swal.fire({ toast: true, position: "top-end", icon: "error", title: "Gagal menghapus member", showConfirmButton: false, timer: 1000, timerProgressBar: true });
      } finally {
